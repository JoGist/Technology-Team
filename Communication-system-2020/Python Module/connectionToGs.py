...
class socket:                                               # nome discutibile, si puo' sempre cambiare
    def __init__(self):                                     # e' attraverso la init che il rover si connette
        self.rover = socketio.Client()                           # un'istanza client di Socket.io
        nm=nmap.PortScanner()
        self.isConnected = False                            # di default non c'è connessione
        thread = threading.Thread(target=self.connectThread, args=[nm, self.rover])          # scelta della funzione che verrà eseguita in background
        thread.start()                                      # avvio del thread
        @self.rover.on('dataToRover')                            # gestisce l'evento dataToRover con la funzione dataToRover
        ...
        @self.rover.on('disconnect')                             # gestisce la disconnessione della grounstation
        def onDisconnect():
            self.rover.disconnect()                              # se non si chiude il socket alla successiva riconessione della GS si riconnetterà
            self.isConnected = False                        # automaticamente in modi imprevedibili, quindi meglio che ce la gestiamo noi
            restartThread = threading.Thread(target=self.connectThread, args=[nm, self.rover])   # sono costretto a definire un altro thread per regole oscure di python
            restartThread.start()

    def sendData(self, object ):                        # object e' proprio un oggetto
        self.rover.emit("dataToGS",json.dumps(object))       # che viene convertito in JSON qui e inviato
    def disconnect(self):
        ...
        while not self.isConnected:
            ...
            for ip in scan['scan'].keys():                          # sulla porta scelta in maniera aggressiva (T5)
                if scan['scan'][ip]['tcp'][port]['state']=='open':  # il risultato è un oggetto dove da qualche parte c'è l'ip con la porta aperta scelta
                    self.rover.connect("http://"+ip+":"+str(port))
